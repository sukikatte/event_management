...
public class EventService {
    ...
    public ErrorType addVenue(String name, String address, int capacity, String contactName, int contactNumber, String contactEmail) {
        Venue v = venueRepository.findByName(name);
        if (v != null){
            return ErrorType.VENUE_OVERLAP;
        }
        Venue venue = new Venue(name, address, capacity, contactName, contactNumber, contactEmail);
    ...
    public ErrorType modifyEvent(ModifyEventDTO edto, Event event){
        ...
        event.setStandingNumberAvailable(edto.getStandingNumberAvailable());
        event.setSeatingNumberAvailable(edto.getSeatingNumberAvailable());
        event.setPremiumNumberAvailable(edto.getPremiumNumberAvailable());
        eventRepository.save(event);
        return ErrorType.NONE;
    }
    ...
    public List<Ticket> buyTicket(long accountId, long eId, TicketType ticketType, int ticketNum) {
        ...
        if(ticketType.equals(TicketType.STANDING)){
            modifyEvent(new ModifyEventDTO(e.getName(), e.getDescription(), e.getDate(), e.getStartTime(), e.getEndTime(), e.getStandingNumberAvailable()-ticketNum, e.getSeatingNumberAvailable(), e.getPremiumNumberAvailable()), e);
        } else if (ticketType.equals(TicketType.SEATING)) {
            modifyEvent(new ModifyEventDTO(e.getName(), e.getDescription(), e.getDate(), e.getStartTime(), e.getEndTime(), e.getStandingNumberAvailable(), e.getSeatingNumberAvailable()-ticketNum, e.getPremiumNumberAvailable()), e);
        }else if (ticketType.equals(TicketType.PREMIUM)) {
            modifyEvent(new ModifyEventDTO(e.getName(), e.getDescription(), e.getDate(), e.getStartTime(), e.getEndTime(), e.getStandingNumberAvailable(), e.getSeatingNumberAvailable(), e.getPremiumNumberAvailable()-ticketNum), e);
        }
